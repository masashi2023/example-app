<div x-data="inputFormHandler()" class="my-2">
    {{-- templateタグ javascriptと一緒に使う　非表示 --}}
    <template x-for="(field,i) in fields" :key="i">
        <div class="w-full flex my-2" x-effect="console.log(fields)">
            <label :for="field.id" class="border border-gray-300 rounded-md p-2 w-full bg-white cursor-pointer">
                <input :id="field.id" type="file" name="images[]" class="sr-only" @change="fields[i].file = $event.target.files[0]"><span class="text-gray-700" x-text="field.file ? field.file.name : '画像を選択'" ></span>
            </label>
            <button type="reset" class="p-2" @click="removeField(i)">
